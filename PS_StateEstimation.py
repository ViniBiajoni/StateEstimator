...
def res_matrix(std_dev,num_meas,type): #Construct R matrix
    ...
    return R
#----------------------------------------------------------------------------------------------------------------------------------
def medidas_h(x_new,meas,Ybus,num_meas,num_states,teta,V,num_bus,shunt_bus):
    ...
    for i in range(num_meas):
        ...
        if meas[i,2]==3:#Pij
            ...
            h[i]= -(V[de-1]**2)*G[de-1,para-1] - V[de-1]*V[para-1]*(-G[de-1,para-1]*np.cos(teta[de-1]-teta[para-1]) -B[de-1,para-1]*np.sin(teta[de-1]-teta[para-1]))
        
        if meas[i,2]==4:#Qij
            h[i]=-(V[de-1]**2)*(-B[de-1,para-1] + shunt_bus[de-1,para-1]) - V[de-1]*V[para-1]*(-G[de-1,para-1]*np.sin(teta[de-1]-teta[para-1]) + B[de-1,para-1]*np.cos(teta[de-1]-teta[para-1]))
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def jacobian(x_k,Ybus,meas,num_states,num_meas,teta,V,num_bus,shunt_bus):        
        ...
        if tipo==3: #Pij
            ...
            for v in range(np.size(barras_voltage, axis = 0)):
                ...
                if de == barras_voltage[v]:
                    ...
                    H[i,col]= -V[para-1]*(-G[de-1,para-1]*np.cos(teta[de-1]-teta[para-1]) - B[de-1,para-1]*np.sin(teta[de-1]-teta[para-1])) - 2*G[de-1,para-1]*V[de-1]   
        ...
        if tipo==4: #Qij
            ...
            for v in range(np.size(barras_voltage, axis = 0)):
                ...
                if de == barras_voltage[v]:
                    ...
                    H[i,col]= -V[para-1]*(-G[de-1,para-1]*np.sin(teta[de-1]-teta[para-1]) + B[de-1,para-1]*np.cos(teta[de-1]-teta[para-1])) - 2*V[de-1]*(-B[de-1,para-1] + shunt_bus[de-1,para-1])  
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------
def  evalue_state(x_k,Ybus,std_dev,meas,num_states,num_meas,R,teta,V,num_bus,z,shunt_line):
    x_new = np.zeros(num_states)
    h= medidas_h(x_new,meas,Ybus,num_meas,num_states,teta,V,num_bus,shunt_line)
    H =jacobian(x_k,Ybus,meas,num_states,num_meas,teta,V,num_bus,shunt_line)
    #Gain Matrix
...
def SE():
    ...
    # Ybus
    Ybus,shunt_line= Y_bus(top,shunt)
    # Main Estimator
    tol=10e-4
    ...
    while True:
        iter = iter + 1
        delta_x,h,H,J = evalue_state(x_k,Ybus,std_dev,meas,num_states,num_meas,R,teta,V,numbus,z,shunt_line)
        Ji.append(J)
